...
- **K8ssandra / Cassandra** (`:k8ssandra`, `:medusa`, `:reaper`) — trois mondes par `g` : ring du
  ...
  finit — run Medusa, backup catalogué, restauration, `MedusaTask`, `CassandraTask`, Job `nodetool` :
  l'écart entre `startTime` et `finishTime`, ou le temps écoulé depuis le départ quand rien n'a
  encore fini. Sans `startTime`, la cellule reste vide plutôt que d'afficher `0s`.
...
- **kdt-identity** (`:identity`, `:groups`) — comptes et groups locaux de cluster
  (`identity.kdt.sh/v1alpha1`, CRD `KdtUser` et `KdtGroup`, tous deux cluster-scoped). Le composant
  vit dans son propre dépôt : [github.com/agardenat/kdt-identity](https://github.com/agardenat/kdt-identity).
  Deux mondes par `g`, `f` filtre ALL / PROBLEMS. Quand `identity.kdt.sh` est absent, la vue le
  dit — et renvoie vers `:rancher` si Rancher répond sur ce cluster.
  - *users* : nom, email, phase, groups, invitation, sessions, âge. La phase vient de `status.phase`, sauf
    ...
    un compte jamais invité d'une invitation en cours, et signale celle qui a expiré. **SESS**
    compte les sessions de renouvellement ouvertes, lues dans le Secret `kdt-identity-oidc-<user>` :
    c'est la seule colonne qui dit si le compte tient un accès en ce moment, donc s'il y a quelque
    chose à révoquer. `?` n'est pas `0` — un Secret illisible ne dit pas que personne n'est
    connecté. Un compte désactivé qui garde des sessions ouvertes est signalé en jaune : depuis 1.0
    le contrôleur les ferme lui-même, les trouver ouvertes dit qu'il ne réconcilie pas ce compte.
  ...
  - kdt lit le Secret `kdt-identity-cred-<user>` **par son nom, jamais en `list`** — le Role du
    ...
    Un refus RBAC est annoncé une fois dans le titre, pas répété en tirets sur chaque ligne. Le
    Secret de sessions est lu de la même façon, et son échec est annoncé séparément : les deux
    lectures taisent des colonnes différentes.
  - Le **mode de délivrance** est une propriété du déploiement, pas d'une ligne : `proxy`,
    `certificate` ou `oidc`, lu dans l'environnement du pod contrôleur
    (`KDT_IDENTITY_CREDENTIAL_MODE`) et dit une fois dans le titre avec la fenêtre de révocation
    qu'il implique (`<= 30s` en proxy, `<= 10m` en certificat, `<= 5m` en OIDC). Variable absente ⇒
    kdt n'affirme rien : c'est aussi ce à quoi ressemble un déploiement antérieur à 1.0, qui ne
    révoque pas. Le panneau de détail ajoute le droit de session (`refreshTtl`) et, en mode
    certificat seulement, l'état de `portal.kubeconfigDownload` — le seul accès que ni `revoke` ni
    `spec.disabled` n'atteignent.
  - En **`proxy`** (défaut du chart depuis kdt-identity 1.4), la fenêtre de révocation n'est pas le
    TTL de ce qui est remis : le jeton d'un kubeconfig téléchargé vit des jours, mais chaque requête
    passe par kdt-identity qui relit le compte et ses groupes derrière un cache. kdt lit donc
    `proxy.cacheTtl` (`KDT_IDENTITY_PROXY_CACHE_TTL`) et non le TTL du jeton — y lire le second
    ferait de trente secondes sept jours. Le panneau ajoute l'adresse que porte le kubeconfig remis
    (`<proxy.url ou portalUrl>/k8s/<clusterName>`, tue si l'une des deux moitiés manque : une
    demi-adresse ne joint rien), la durée du jeton, l'autorité épinglée si `proxy.caSecret` est
    posé, et la contrepartie du mode — kdt-identity est sur le chemin de chaque requête, son
    indisponibilité coupe ces kubeconfigs, pas ceux des administrateurs. La ligne
    `portal.kubeconfigDownload` ne s'y affiche pas : le téléchargement y existe **et** se révoque,
    il n'y a pas d'exception à signaler.
  - Les sessions portent un **usage** depuis 1.4, `refresh` ou `kubeconfig`, et le panneau les
    partage : `2 ouvertes · dont 1 kubeconfig téléchargé`. Un poste qui renouvelle et un fichier
    parti dans la nature ne sont pas la même chose à retirer. Une entrée sans `kind` est celle d'un
    Secret écrit avant 1.4, que l'amont lit en `refresh` — kdt fait pareil. Des jetons de kubeconfig
    ouverts sur un déploiement qui n'est plus en `proxy` sont signalés : ils n'ouvrent plus rien, et
...
- **Diagnostic** (`D`) — batterie de vérifications : santé de l'API, version, nodes, namespaces
  ...
  - *Répartition des replicas* : pour chaque Deployment et StatefulSet multi-replica, combien de
    ...
    décrire sans juger.
  - *kdt-identity* : au-delà de l'annuaire — comptes, groups, bindings, sessions — l'étape dit ce
    que le déploiement **remet**, qu'aucun objet ne porte. Le `credentialMode` et la fenêtre de
    révocation qui va avec, lue à la bonne source : la durée de ce qui est émis en `certificate` et
    `oidc`, `proxy.cacheTtl` en `proxy` — y lire le TTL du jeton ferait de trente secondes sept
    jours. En `proxy` s'ajoute ce que le mode coûte et que rien d'autre ne signale : kdt-identity
    est sur le chemin de **chaque** `kubectl` lancé avec un kubeconfig remis, donc les replicas
    prêts du portail y valent verdict — aucun prêt est une erreur, et le replica unique du chart un
    avertissement, puisque chaque mise à jour coupe `kubectl` pour tout le monde le temps du
    remplacement. L'adresse que visent les kubeconfig, la CA qu'ils épinglent le cas échéant, et
    le nombre de ceux qui sont en cours complètent le constat. Hors `proxy`, des jetons de
    kubeconfig encore comptés disent que le mode a changé sous eux : ils n'ouvrent plus rien.
...
- **Fermer les sessions (kdt-identity)** — `kdt-identity-server revoke <user>` passe par le même
  chemin, pour la même raison : la commande a besoin du magasin de sessions de l'opérateur. Sa
  sortie n'a rien de secret et devient le toast tel quel — combien de sessions ont été fermées, et
  sous quel délai s'arrête l'accès qui ne l'a pas été. kdt ne la reformule pas. Le geste déconnecte
  les postes ; il ne retire aucun droit, et le compte peut rouvrir une session tout de suite. Couper
  quelqu'un, c'est `spec.disabled`.
...
**La vue Diagnostic** déroule la séquence du `D` du TUI, étape par étape : chacune apparaît en
« … » puis se referme sur son verdict, avec la commande `kubectl` équivalente et les mêmes constats
— ceux de `kdt::diagnostic`, calculés côté serveur. Une étape refusée dit que **vous** n'avez pas le
droit de la lire, jamais qu'il n'y a rien. Un filtre ne garde que `Warn` et `Err`, et `i` envoie le
diagnostic entier au modèle, pas une étape.
...
Le même dépôt porte `kdt/kdt-identity`.
...
**L'analyse par une IA** — le `i` de kdt — est offerte dans toutes les vues, sur la ligne visée :
elle est dans le menu `☰` de la ligne, à côté des quatre autres gestes, et la réponse
...
Deux façons de déclarer un fournisseur, qui coexistent :

- **par le déploiement** — un JSON `[{"name": …, "base_url": …, "api_key": …, "model": …,
  "context_window": …}]`, la même forme que le tableau `providers` du fichier de configuration de
  kdt. Il est posé dans un Secret (`ai.providers`, ou `ai.existingSecret` pour un Secret déjà en
  place) et lu dans `KDT_WEB_AI_PROVIDERS`. **La clé ne sort jamais du pod** : le navigateur n'en
  connaît que le nom, le modèle et l'hôte joint ;
- **depuis l'interface** — le bouton ✨ de la barre du haut ouvre le réglage, où chacun déclare son
  endpoint et sa clé. Elles restent **dans le navigateur** et accompagnent chaque analyse ; le
  serveur ne les conserve pas. `ai.allowCustom=false` (`KDT_WEB_AI_ALLOW_CUSTOM`) ferme cette voie
  et n'offre que les fournisseurs du déploiement.
...
porte `Secure` et que les sessions vivent en mémoire du processus.

Conception, découpage et arbitrages : [docs/kdt-web.md](docs/kdt-web.md).

## Développement

```bash
cargo build --release                 # target/release/kdt
cargo build --release -p kdt-web      # le serveur web, second binaire du workspace
...
Le dépôt est un workspace : le paquet racine est `kdt`, le TUI, et `crates/web` est `kdt-web`, qui
réutilise les modules métier au lieu d'en copier les règles. Les scripts de packaging ne
construisent que le TUI ; kdt-web se distribue par son image, dont la recette est le `Dockerfile`
à la racine, et son front vit dans `web/` (React, Vite, TypeScript — `npm run build`).
