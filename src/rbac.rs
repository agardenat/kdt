...
pub enum Provenance {
    ...
    Argo { app: String },
    // Generated by a Kyverno `generate` rule: the object carries the policy that made it.
    Kyverno { policy: String },
    // Projected by Rancher's RBAC controllers from a RoleTemplateBinding / GlobalRoleBinding.
    Rancher { binding: String },
    // Shipped by the API server itself (`kubernetes.io/bootstrapping: rbac-defaults`).
    Bootstrap,
    // Installed and reconciled by a cluster addon manager (AKS, GKE addon-manager).
    Addon { by: String },
...
impl Provenance {
    pub fn label(&self) -> String {
        match self {
            ...
            Provenance::Kyverno { policy } => format!("kyverno:{policy}"),
            Provenance::Rancher { binding } => format!("rancher:{binding}"),
            Provenance::Bootstrap => "rbac-defaults".to_string(),
            Provenance::Addon { by } => format!("addon:{by}"),
            Provenance::Owner { kind, name } => format!("{kind}:{name}"),
            Provenance::Kubectl => "kubectl".to_string(),
            Provenance::Unmanaged => "unmanaged".to_string(),
        }
    }
    // A grant applied outside GitOps is an audit blind spot worth flagging on risky bindings. A
    // binding a controller owns (Kyverno, Rancher, the API server's own defaults, an addon manager)
    // is not one: it names what recreates it, so drift has somewhere to be read back from.
...
// Kyverno stamps every object a `generate` rule produces with the policy and the rule that made it.
const LBL_KYVERNO_POLICY: &str = "generate.kyverno.io/policy-name";
const LBL_KYVERNO_RULE: &str = "generate.kyverno.io/rule-name";
// Rancher's RBAC controllers project cluster/project role bindings down onto the cluster. Each form
// carries its own marker: `rtb-owner-updated` on the projected RoleBinding, `globalrolebinding` on
// the ClusterRoleBinding behind a GlobalRole, `impersonator` on the per-user impersonation grant.
const LBL_RTB_OWNER_UPDATED: &str = "authz.cluster.cattle.io/rtb-owner-updated";
const LBL_RTB_OWNER: &str = "authz.cluster.cattle.io/rtb-owner";
const LBL_CATTLE_GRB: &str = "authz.management.cattle.io/globalrolebinding";
const LBL_CATTLE_IMPERSONATOR: &str = "authz.cluster.cattle.io/impersonator";
// The API server's own RBAC defaults, and the addon managers that reinstall what they shipped.
const LBL_BOOTSTRAP: &str = "kubernetes.io/bootstrapping";
const LBL_ADDONMANAGER: &str = "addonmanager.kubernetes.io/mode";
const LBL_AZURE_MANAGEDBY: &str = "kubernetes.azure.com/managedby";
...
pub fn detect_provenance(meta: &ObjectMeta) -> Provenance {
    ...
    if let Some(policy) = labels.get(LBL_KYVERNO_POLICY) {
        return Provenance::Kyverno { policy: policy.clone() };
    }
    // A rule with no policy label still names itself, and that is enough to find it again.
    if let Some(rule) = labels.get(LBL_KYVERNO_RULE) {
        return Provenance::Kyverno { policy: rule.clone() };
    }
    if let Some(owner) = labels.get(LBL_RTB_OWNER_UPDATED).or_else(|| labels.get(LBL_RTB_OWNER)) {
        // Value shape is `<project namespace>_<binding name>`; the binding is what one goes to read.
        let binding = owner.split_once('_').map(|(_, b)| b).unwrap_or(owner);
        return Provenance::Rancher { binding: binding.to_string() };
    }
    if labels.contains_key(LBL_CATTLE_GRB) {
        return Provenance::Rancher { binding: "globalrolebinding".to_string() };
    }
    if labels.contains_key(LBL_CATTLE_IMPERSONATOR) {
        return Provenance::Rancher { binding: "impersonation".to_string() };
    }
    // The ClusterRoleBindings Rancher projects for a project role carry no marker key of their own:
    // the binding is the label *key*, shaped `<project namespace>_<binding name>`. Only trust that
    // shape on the generated `rb-`/`crb-` names Rancher gives those objects.
    if let Some(binding) = rancher_projected_binding(meta.name.as_deref(), &labels) {
        return Provenance::Rancher { binding };
    }
    if labels.get(LBL_BOOTSTRAP).map(|v| v == "rbac-defaults").unwrap_or(false) {
        return Provenance::Bootstrap;
    }
    if let Some(by) = labels.get(LBL_AZURE_MANAGEDBY) {
        return Provenance::Addon { by: by.clone() };
    }
    if labels.contains_key(LBL_ADDONMANAGER) {
        return Provenance::Addon { by: "addonmanager".to_string() };
    }
...
// `rb-3uxk6xpuma` / `crb-2hvhc3c5sf`: Rancher names its projected bindings from a hash, and labels
// them `<project namespace>_<binding name>: <role template>`. Match the name first so a label key
// that merely happens to hold an underscore is never read as a Rancher binding.
fn rancher_projected_binding(name: Option<&str>, labels: &Labels) -> Option<String> {
    let name = name?;
    let hash = name.strip_prefix("crb-").or_else(|| name.strip_prefix("rb-"))?;
...
impl Subject {
    ...
    // What a table column can show of that identity. An authenticator hands over the whole string it
    // was configured with — `openldap_group://CN=team,OU=Groups,OU=Hadoop,DC=example,DC=fr` — and end
    // truncation would leave every row reading `grp:openldap_group://CN=`. The leftmost RDN is the
    // name; the rest is where the directory keeps it. Two DNs can share a CN, so this is a display
    // form only: the detail panel keeps the string exactly as the binding spells it.
    pub fn short_label(&self) -> String {
        short_identity(&self.label())
    }
}

// The display form of an already-built subject label, for the tree rows that carry one as a string.
pub fn short_identity(label: &str) -> String {
    let Some((head, dn)) = label.split_once("://") else { return label.to_string() };
    match leftmost_rdn(dn) {
        Some(rdn) => format!("{}:{}", head.split_once(':').map(|(p, _)| p).unwrap_or(head), rdn),
        None => label.to_string(),
    }
}

// `CN=team,OU=Groups,DC=fr` -> `team`. `None` when the string is not shaped like a DN at all, so an
// identity that merely contains a `://` is never silently rewritten.
fn leftmost_rdn(dn: &str) -> Option<&str> {
    let (attr, rest) = dn.split_once('=')?;
...
impl RbacBinding {
    ...
    // The worst tag alone, for the RISK column. Spelling out every tag on every row printed the same
    // red words down the whole table and left nothing to tell two grants apart; `findings` is sorted
    // worst first, and the full list is one cursor move away in the detail panel.
    pub fn risk_top(&self) -> String {
        match self.findings.split_first() {
            None => "read-only".to_string(),
            Some((first, [])) => first.tag.to_string(),
            Some((first, rest)) => format!("{} +{}", first.tag, rest.len()),
        }
    }
}
...
mod tests {
    ...
    fn meta(name: &str, pairs: &[(&str, &str)]) -> ObjectMeta {
        ObjectMeta {
    ...
    #[test]
    fn kyverno_generated_binding_names_its_policy() {
        let m = meta(
            "ns-owner-binding",
            &[
                ("app.kubernetes.io/managed-by", "kyverno"),
                ("generate.kyverno.io/policy-name", "generate-ns-owner-rbac"),
                ("generate.kyverno.io/rule-name", "generate-ns-owner-rbac"),
            ],
        );
        let p = detect_provenance(&m);
        assert_eq!(p, Provenance::Kyverno { policy: "generate-ns-owner-rbac".into() });
        assert_eq!(p.label(), "kyverno:generate-ns-owner-rbac");
        // A controller recreates it: attributed, so not the blind spot a hand-applied grant is.
        assert!(!p.out_of_gitops());
    }

    #[test]
    fn rancher_rtb_owner_keeps_the_binding_not_the_project_namespace() {
        let m = meta(
            "rb-ftsv3g4cne",
            &[(
                "authz.cluster.cattle.io/rtb-owner-updated",
                "local-p-f4gcw_dsm-ppl-aks-owner-project-owner",
            )],
        );
        assert_eq!(
            detect_provenance(&m),
            Provenance::Rancher { binding: "dsm-ppl-aks-owner-project-owner".into() }
        );
    }

    #[test]
    fn rancher_projected_binding_reads_its_label_key() {
        // The only marker on these is the label *key*; the value is the role template.
        let m = meta("crb-2hvhc3c5sf", &[("local-p-5h26x_creator-project-owner", "owner-user")]);
        assert_eq!(
            detect_provenance(&m),
            Provenance::Rancher { binding: "creator-project-owner".into() }
        );
    }

    #[test]
    fn an_underscore_in_a_label_key_is_not_a_rancher_binding() {
        // Same label shape, but the name is not one Rancher generates: stay honest.
        let m = meta("my-team-binding", &[("some_key", "v")]);
        assert_eq!(detect_provenance(&m), Provenance::Unmanaged);
    }

    #[test]
    fn api_server_defaults_are_not_out_of_gitops() {
        let m = meta("cluster-admin", &[("kubernetes.io/bootstrapping", "rbac-defaults")]);
        let p = detect_provenance(&m);
    ...
    #[test]
    fn flux_still_wins_over_every_controller_marker() {
        let m = meta(
            "x",
            &[
                ("kustomize.toolkit.fluxcd.io/name", "apps"),
                ("kustomize.toolkit.fluxcd.io/namespace", "flux-system"),
                ("generate.kyverno.io/policy-name", "p"),
            ],
        );
        assert_eq!(
            detect_provenance(&m),
            Provenance::FluxKustomization { namespace: "flux-system".into(), name: "apps".into() }
        );
    }
    ...
    #[test]
    fn a_group_dn_shows_its_leftmost_rdn() {
        let s = Subject {
            kind: "Group".into(),
            name: "openldap_group://CN=team-owner,OU=Groups,OU=Hadoop,DC=example,DC=fr".into(),
            namespace: None,
        };
        assert_eq!(s.short_label(), "grp:team-owner");
        // The detail panel still gets the string exactly as the binding spells it.
        assert!(s.label().ends_with("DC=fr"));
    }
    ...
    fn a_plain_subject_is_left_alone() {
        ...
        assert_eq!(s.short_label(), "sa:ci/builder");
        assert_eq!(short_identity("user:u-oxivhzvbov"), "user:u-oxivhzvbov");
        // A `://` that is not followed by a DN is not rewritten into something shorter but wrong.
        assert_eq!(short_identity("user:https://issuer/sub"), "user:https://issuer/sub");
    }
